"""
orchestrator.py  (v2 — with arb layer as gate 4)
...
Every SCAN_INTERVAL_SEC:
  ...
  4. ArbLayer: cross-venue check vs Polymarket / OddsPapi  (gate 4)
  5. Kelly size + place order + log trade

Nightly at RETRAIN_HOUR:
  6. Feed resolved contracts back into Bayesian models
  7. Recompute calibration — Brier score updates bot weights
...
from shared.calibration_logger import init_db, log_trade, compute_calibration
...
class FlywheelOrchestrator:
    ...
    def _evaluate_market(self, market: dict) -> None:
        ...
        if not arb.passes:
            logger.info(
                "ARB GATE VETO %s | dir_aligned=%s spread=%.2%% notes=%s",
                ticker, arb.direction_aligned, arb.cross_spread, arb.notes,
            )
            return
        ...
        if arb.sharp_line_prob is not None:
            logger.info(
                "SHARP CONFIRM %s | kalshi=%.2%% sharp=%.2%% spread=%+.2%%",
                ticker, yes_price / 100, arb.sharp_line_prob,
                (yes_price / 100) - arb.sharp_line_prob,
            )
        ...
        if consensus.direction == "YES":
            sizing = kelly_stake(
                prob=consensus.avg_prob, yes_price_cents=yes_price,
                bankroll=self.bankroll, sector=lead_sector,
        ...
        try:
            order_resp = self.client.place_order(
                ...
                count=sizing["contracts"], yes_price=yes_price,
        ...
        trade_id = log_trade(
            ...
            contracts=sizing["contracts"], yes_price_cents=yes_price,
            dollars_risked=sizing["dollars"], avg_prob=consensus.avg_prob,
            avg_edge=consensus.avg_edge, avg_confidence=consensus.avg_confidence,
            order_id=order_id, demo_mode=DEMO_MODE,
        )

        # Build trade log line
        arb_note = f" arb_spread={arb.cross_spread:.2%}" if arb.arb_exists else ""
        logger.info(
            "TRADE #%d | %s | %s %dx@%dc | $%.2f | edge=%+.2%% conf=%.0f%%%s",
    ...
    def run(self) -> None:
        logger.info("Kalshi Flywheel v2 | DEMO=%s | $%.2f | arb_mode=%s",
